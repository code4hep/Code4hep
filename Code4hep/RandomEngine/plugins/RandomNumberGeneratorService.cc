...
// Created: March 7, 2006 (originally in FWCore/Services)
//
...
#include "Code4hep/RandomEngine/cloneEngine.h"
#include "Code4hep/RandomEngine/RandomEngineState.h"
...
private:
  ...
  static const std::uint32_t maxSeedMixMax;
};
...
const std::uint32_t RandomNumberGeneratorService::maxSeedMixMax = 4294967295U;
  ...
  for (auto const& label : pSets) {
    ...
    // For the other engines, one seed is required
      ...
      if (initialSeedSet.size() != 1U) {
        throw Exception(errors::Configuration)
            << "Random engines of type \"HepJamesRandom\" and \"MixMaxRng\" \n"
      ...
      if (engineName == "HepJamesRandom") {
        ...
        if (initialSeedSet[0] > maxSeedMixMax) {
          throw Exception(errors::Configuration)
              << "The CLHEP::MixMaxRng engine seed should be in the range 0 to " << maxSeedMixMax << ".\n"
              << "The seed passed to the RandomNumberGenerationService from the\n"
                 "configuration file was "
              << initialSeedSet[0] << ".  This was for \n"
              << "the module with label " << label << ".\n";
  ...
  if (iter == moduleIDVector.end() || iter->moduleID() != moduleID) {
    throw Exception(errors::Configuration)
        ...
        << " = cms.PSet(\n"
           ...
           "    engineName = cms.untracked.string('Mixmax')\n"
           "  )\n"
    ...
    if (moduleID != std::numeric_limits<unsigned int>::max()) {
      ...
      // For the other engines, one seed is required
        ...
        if (name == "HepJamesRandom") {
          ...
          throw cms::Exception("LogicError") << name << " is not supported";
        }
      }
...
void RandomNumberGeneratorService::resetEngineSeeds(LabelAndEngine& labelAndEngine,
  ...
  if (engineName == "RanecuEngine") {
    ...
    if (engineName == "HepJamesRandom" || engineName == "MixMaxRng") {
      ...
      std::uint32_t max32 = maxSeedMixMax;
...
typedef edm::serviceregistry::AllArgsMaker<edm::RandomNumberGenerator, edm::service::RandomNumberGeneratorService> RandomMaker;

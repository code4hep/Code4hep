...
PodioOutputModule::PodioOutputModule(edm::ParameterSet const& iPSet):
  ...
  writer_{podio::makeWriter(iPSet.getUntrackedParameter<std::string>("fileName"), iPSet.getUntrackedParameter<std::string>("type"))} {
}
...
void PodioOutputModule::fillDescriptions(edm::ConfigurationDescriptions& oDesc) {
  ...
  pset.addUntracked<std::string>("type", "default")->setComment("The type of the file to write, e.g. 'root'. The value of 'default' says the type should be based on the file extension in the name.");
  oDesc.addDefault(pset);
}

  bool PodioOutputModule::finalSelection(edm::ProductDescription const& product) const {
    try {
      auto const& typeName = product.fullClassName();
      auto factory = code4hep::CollectionWrapperConverterBaseFactory::get();
      (void) factory->create(typeName);
    } catch(cms::Exception const&) {
      edm::LogWarning("UnstorableType")<<"The type '"<<product.fullClassName()<<"' is not regiersted with the C4H_COLLECTION macro so can not be stored. It will not be consumed by this module.";
      return false;
    }  
    return true;
  }
...
void PodioOutputModule::write(edm::EventForOutput const& e) {
  ...
  for(auto const& product: keptProducts()[edm::InEvent]) {
    auto const& typeName = product.first->fullClassName();
